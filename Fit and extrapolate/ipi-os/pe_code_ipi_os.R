...
modFP1_1 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(-2))+ offset(log(timedelta)) , family=binomial(link=cloglog), data=ltHaz)
modFP1_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(-1))+ offset(log(timedelta)) , family=binomial(link=cloglog), data=ltHaz)
modFP2_1 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(-0.5)) + I(Time^0*lnTime) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz)
modFP2_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(-1)) + I(Time^0.5) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz)
...
write.csv(dfGOF,"ipi-3y-os-AIC.csv")
...
write.csv(dfHazEst,"dfhazest_ipios_3y.csv")
write.csv(ltHaz,"lthaz_ipios_3y.csv")
...
ggsave("fig_haz_ipi_3y_os.png",plot = fig_haz_ipi_2y,width = 18,height = 12,dpi = 800)
...
ggplot(data=LE_5y_predict,mapping=aes(x=Model,y=LE,fill=LE,group=factor(1)))+
  ...
  expand_limits(y=c(5.8,6))+
  scale_y_continuous(breaks = c(seq(from=5.8, to=6,by = 0.02)))+
...
ggplot(data=LE_2y,mapping=aes(x=Model,y=LE,fill=LE,group=factor(1)))+
  geom_bar(stat="identity",width=0.8)+
  geom_text(aes(label = round(LE,digits = 3), vjust = -0.8, hjust = 0.5), show.legend = TRUE)+
  expand_limits(y=c(2.8,2.85))+
  scale_y_continuous(breaks = c(seq(from=2.8, to=2.85,by = 0.01)))+
  coord_cartesian(ylim = c(2.8,2.85))
...
ggsave("fig_sse_ipi_3y_os.png",plot = sse_plot,width = 18,height = 12,dpi = 800)
...
ggsave("fig_sae_ipi_3y_os.png",plot = sae_plot,width = 18,height = 12,dpi = 800)
...
write.csv(dfsurvLE,"ipi-os-3y-LE.csv")
...
modFP1_1 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^1)+ offset(log(timedelta)) , family=binomial(link=cloglog), data=ltHaz_out)
modFP1_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^2)+ offset(log(timedelta)) , family=binomial(link=cloglog), data=ltHaz_out)
modFP2_1 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(-1)) + I(Time^1) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out)
modFP2_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(-0.5)) + I(Time^1) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out)
...
write.csv(dfGOF_out,"ipi-6.5y-os-AIC.csv")

write.csv(ltHaz_out,"lthaz_nivpfs_3y.csv")
########==============================================================================================###
...
ggsave("fig_haz_ipi_6.5y_os.png",plot = fig_haz_niv_5y,width = 18,height = 12,dpi = 800)
...
write.csv(ltHaz_out,"lthaz_ipios_6.5y.csv")
...
write.csv(dfsurvLE2,"ipi-os-6.5y-LE.csv")
