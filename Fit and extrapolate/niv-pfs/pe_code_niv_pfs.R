...
modFP1_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^0*lnTime)+ offset(log(timedelta)) , family=binomial(link=cloglog), data=ltHaz)
...
write.csv(dfHazEst,"dfhazest_nivpfs_3y.csv")
write.csv(ltHaz,"lthaz_nivpfs_3y.csv")
########==============================================================================================###
...
ggsave("fig_haz_niv_3y_pfs.png",plot = fig_haz_ipi_2y,width = 18,height = 12,dpi = 800)
...
ggplot(data=LE_5y_predict,mapping=aes(x=Model,y=LE,fill=LE,group=factor(1)))+
  ...
  expand_limits(y=c(5.7,6.1))+
  scale_y_continuous(breaks = c(seq(from=5.7, to=6.1,by = 0.1)))+
  coord_cartesian(ylim = c(5.7,6.1))


ggplot(data=LE_2y,mapping=aes(x=Model,y=LE,fill=LE,group=factor(1)))+
  geom_bar(stat="identity",width=0.8)+
  geom_text(aes(label = round(LE,digits = 3), vjust = -0.8, hjust = 0.5), show.legend = TRUE)+
  expand_limits(y=c(2.7,2.9))+
  scale_y_continuous(breaks = c(seq(from=2.7, to=2.9,by = 0.01)))+
  coord_cartesian(ylim = c(2.7,2.9))
...
ggsave("fig_sse_niv_3y_pfs.png",plot = sse_plot,width = 18,height = 12,dpi = 800)
...
write.csv(dfsurvLE,"niv-pfs-3y-LE.csv")
...
modFP2_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(-1)) + I(Time^0*lnTime) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out)
...
write.csv(dfGOF_out,"niv-6.5y-pfs-AIC.csv")

write.csv(ltHaz_out,"lthaz_nivpfs_6.5y.csv")
...
fig1a <- ggplot(data=filter(dfFig_out, Model %in% c("exp","weibull","gamma","lognormal","gompertz","loglogistic","gengamma","genf")), aes(x=Time, y=Haz)) +
  ...
  labs(x="Time", y="Hazard")  + guides(size="none") +
  theme(legend.position = "bottom") + coord_cartesian(ylim=c(0,0.2), xlim=c(0,7)) 
#+ theme(axis.text.y = element_blank())
#fig1a
...
ggsave("fig_haz_niv_6.5y_pfs.png",plot = fig_haz_niv_5y,width = 18,height = 12,dpi = 800)
...
ggsave("fig_surv_niv_6.5y_pfs.png",f_surv_out,width = 18,height = 12,dpi = 800)
...
write.csv(dfsurvLE2,"niv-pfs-6.5y-LE.csv")
