...
modFP1_1 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(1))+ offset(log(timedelta)) , family=binomial(link=cloglog), data=ltHaz)
modFP1_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(2))+ offset(log(timedelta)) , family=binomial(link=cloglog), data=ltHaz)
...
modFP2_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(1)) + I(Time^0*lnTime) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz)
...
for (i in 1:3){
  for (j in 0:5){
      MyTemp <- flexsurvspline(Surv(recyrs, censrec) ~ 1, data = sd_bc, k = j, scale = MyScale[[i]])
...
write.csv(dfGOF,"niv-3y-os-AIC.csv")
...
write.csv(dfHazEst,"dfhazest_nivos_3y.csv")
write.csv(ltHaz,"lthaz_nivos_3y.csv")

########==============================================================================================###
dfHazEst<-read.csv("dfhazest_nivos_3y.csv")
dfHazEst<-dfHazEst[,2:79]
ltHaz<-read.csv("lthaz_nivos_3y.csv")
ltHaz<-ltHaz[,2:31]
...
fig_haz_new <- ggplot(data=filter(dfFig, Model %in% c("exp","weibull","gamma","lognormal","gompertz","loglogistic","gengamma","genf",
  ...
  labs(title="Hazard plot of NI group",
       subtitle = "3Ys")
fig_haz_new
ggsave("haz_niv3y_os_new.png",plot = fig_haz_new,width = 18,height = 12,dpi = 1200)
...
ggsave("fig_haz_niv_3y_os.png",plot = fig_haz_ipi_2y,width = 18,height = 12,dpi = 800)
...
ggplot(data=LE_5y_predict,mapping=aes(x=Model,y=LE,fill=LE,group=factor(1)))+
  ...
  expand_limits(y=c(6.1,6.3))+
  scale_y_continuous(breaks = c(seq(from=6.1, to=6.3,by = 0.1)))+
  coord_cartesian(ylim = c(6.1,6.3))


ggplot(data=LE_2y,mapping=aes(x=Model,y=LE,fill=LE,group=factor(1)))+
  geom_bar(stat="identity",width=0.8)+
  geom_text(aes(label = round(LE,digits = 3), vjust = -0.8, hjust = 0.5), show.legend = TRUE)+
  expand_limits(y=c(2.8,2.9))+
  scale_y_continuous(breaks = c(seq(from=2.8, to=2.9,by = 0.01)))+
  coord_cartesian(ylim = c(2.8,2.9))
...
#SD1
SSE<-data.frame(matrix(nrow=20,ncol=2))
...
ggsave("fig_sse_niv_3y_os.png",plot = sse_plot,width = 18,height = 12,dpi = 800)
...
SAE_surv<-dfsurv[37:78,2:21]
colnames(SAE_surv)<-modnames
#SD1
SAE<-data.frame(matrix(nrow=20,ncol=2))
...
dfsurvLE<-as.data.frame(array(dim=c(480,21)))
...
write.csv(dfsurvLE,"niv-os-3y-LE.csv")
...
modFP1_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^0*lnTime)+ offset(log(timedelta)) , family=binomial(link=cloglog), data=ltHaz_out)
modFP2_1 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(-2)) + I(Time^0.5) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out)
modFP2_2 <- glm(cbind(Events,AtRisk-Events) ~ I(Time^(-1)) + I(Time^0.5) + offset(log(timedelta)), family=binomial(link=cloglog), data=ltHaz_out)
...
for (i in 1:3){
  for (j in 0:5){
    MyTemp <- flexsurvspline(Surv(recyrs, censrec) ~ 1, data = sd_bc_out, k = j, scale = MyScale[[i]])
    MyAIC[[(i-1)*6+1+j]] <- (-2*MyTemp$loglik+2*MyTemp$npars)
  }
}
MyAICResults <- as.data.frame(cbind(seq(1:6)-1,MyAIC))
...
write.csv(dfGOF_out,"niv-6.5y-os-AIC.csv")

write.csv(ltHaz_out,"lthaz_nivos_6.5y.csv")
########==============================================================================================###

ltHaz_out<-read.csv("lthaz_nivos_6.5y.csv")
...
dfsurvLE2<-as.data.frame(array(dim=c(480,21)))
...
write.csv(dfsurvLE2,"niv-os-6.5y-LE.csv")
