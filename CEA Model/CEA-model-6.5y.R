...
setwd("C:/Users/ECHO/Desktop/PE-flexible/CEA-MODEL/LE-data")

niv_pfs<-read.csv("niv-pfs-6.5y-LE.csv")
niv_os<-read.csv("niv-os-6.5y-LE.csv")
ipi_pfs<-read.csv("ipi-pfs-6.5y-LE.csv")
ipi_os<-read.csv("ipi-os-6.5y-LE.csv")

niv_os<-data.frame(niv_os$Time,niv_os$gengamma,niv_os$FP2.1,niv_os$RCS1,niv_os$RP.normal.1)
niv_pfs<-data.frame(niv_pfs$Time,niv_pfs$genf,niv_pfs$FP2.1,niv_pfs$RCS2,niv_pfs$RP.odds.2)
ipi_os<-data.frame(ipi_os$Time,ipi_os$genf,ipi_os$FP2.1,ipi_os$RCS2,ipi_os$RP.odds.1)
ipi_pfs<-data.frame(ipi_pfs$Time,ipi_pfs$gengamma,ipi_pfs$FP2.1,ipi_pfs$RCS2,ipi_pfs$RP.odds.1)
...
#                 geom_point(aes(y=Incremental_Cost[218] ,x=Incremental_QALY[218]),shape=16,colour="red",size=4)+
#                 scale_fill_continuous()+
#                 stat_function(fun=wtp_line, geom="line",xlim = c(-0.5,3),size=1,colour="hotpink")+
#                 coord_cartesian(ylim=c(-120000,120000),xlim = c(0,2))+
#                 theme_bw()+
...
# coord_cartesian(ylim=c(-400000,150000),xlim = c(0,5))+
# theme_bw()+
